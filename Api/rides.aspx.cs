...
public partial class Api_rides : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (Request.QueryString["offer"] != null)
        {
            ...
            String passenterCount = Request.QueryString["passengers"];
            ...
            con.Open();
            string selectstatement = "INSERT INTO Offer_Information (Offer_ID, UserID, Longitude, Latitude, Distance, StartDate, ExpiryDate, Passenger_Count, Type, Rate, OfferInformation, Address) VALUES(" + (DateTime.Now.Ticks) + ", " + userId + ", " + longitude + ", " + latitude + ", " + distance + " , 0, 0 , " + passenterCount + ", 0, " + rate + " , 0 , 0 )";
    ...
    private Data getLocation(double [] bounds, double radius)
    {
        ...
        string selectstatement = "SELECT offer.*, usertable.Firstname AS user1 FROM Offer_Information AS offer, User_Information AS usertable WHERE offer.UserID = usertable.User_ID";
        SqlCommand selectcommand = new SqlCommand(selectstatement, con);
